...
export default function AlbumForm({ mode, initialData, preview, isLoading, error,
    ...
    return (
        <ModalLayout onClick={handleCancle}>
            <div
                onClick={handleClick}
                className="relative p-2 max-w-[360px] md:max-w-[80vh] md:max-h-[80vh] bg-bcdeepblue rounded-2xl shadow-lg"
            >
                <div className="hidden md:flex flex-col px-14 py-8 justify-center items-center border border-gray-400 rounded-xl">
                    <button 
                ...
                <div className="md:hidden flex flex-col px-14 py-8 justify-center items-center border border-gray-400 rounded-xl">
                    <button 
                        onClick={handleCancle}
                        className="absolute top-2 right-2 p-2 rounded-lg text-white text-2xl hover:text-gray-700"
                    >
                    ...
                    <h1 className="text-4xl text-gray-200 mb-8">{mode === "create" ? "앨범 추가" : "앨범 수정"}</h1>
                    <form id="album-form" onSubmit={handleSubmit} className="flex flex-col items-center justify-between gap-4">
                        <div className="">
                            <label className="flex w-48 h-48 border-2 border-dashed rounded-lg justify-center items-center cursor-pointer bg-gray-400">
                                {preview ? (
                                    <img src={preview} alt="미리보기" className="w-full h-full rounded-lg object-cover" />
                    ...
                    <div className="flex text-xl mt-6 gap-6">
                        <button type="submit" disabled={isLoading} form="album-form"
                            className="px-6 py-4 bg-bclightblue hover:opacity-80 rounded-xl shadow-lg">
                            {mode === "create" ? "저장" : "수정"}
                        </button>
